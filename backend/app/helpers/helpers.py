...
def chunk_text_with_context(text, chunk_size=5, overlap=2):
    ...
    for i in range(0, len(buffer), chunk_size - overlap):
        ...
        chunk_context = " > ".join(
            chunk_lines[0][1]) if chunk_lines else "GENERAL"
        chunks.append({
            "content": chunk_text,
...
def process_document_to_faiss(
    ...
    new_vector_data = build_vector_dataset(
        chunk_texts, contexts, embeddings, source, topic=topic)
    ...
    if index is None:
        ...
        if embeddings.shape[1] != index.d:
            raise ValueError(
                f"Dimension mismatch! Index dim={index.d}, new vectors dim={embeddings.shape[1]}")
...
def load_faiss_index(file_path):
    ...
    gpu_index = faiss.index_cpu_to_gpu(
        res, 0, cpu_index)  # Chuyển sang GPU device 0
